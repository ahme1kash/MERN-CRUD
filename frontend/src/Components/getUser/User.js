...
const User = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            let response = await axios.get("http://localhost:6003/api/getAllUser")
    ...
    const deleteUser = async (userId) => {
        try {
            await axios.delete(`http://localhost:6003/api/delete/${userId}`)
            setUsers((prevUser) => {
    ...
    const deleteAll = async () => {
        try {
            await axios.delete("http://localhost:6003/api/deleteAllUser")
            setUsers([])
